...
export interface UserPereferencesStoreModel {
...
export class UserPereferencesStore extends BaseStore<UserPereferencesStoreModel> /* implements UserStoreFlatModel (when strict null is enabled) */ {
  ...
  @action
  protected fromStore({ lastSeenAppVersion, preferences }: Partial<UserPereferencesStoreModel> = {}) {
  ...
  toJSON(): UserPereferencesStoreModel {
    const model: UserPereferencesStoreModel = {
