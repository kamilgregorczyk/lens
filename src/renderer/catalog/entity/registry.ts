...
export class CatalogEntityRegistry implements BaseCatalogEntityRegistry {
  ...
  getById(id: string): CatalogEntity {
    return this.entityMap.get().get(id);
  }
  ...
  getItemsForApiKind(apiVersion: string, kind: string, { filtered = false } = {}): CatalogEntity[] {
    ...
    return entities.get().filter(entity => entity.apiVersion === apiVersion && entity.kind === kind);
  }
  ...
  getItemsByEntityClass(constructor: CatalogEntityConstructor<CatalogEntity>): CatalogEntity[] {
    return this.entities.get().filter((item) => item instanceof constructor);
  }
